define("src/salesmap/maphelper", [
  "gmaps",
  // define("MapPage", ['$scope', '$http', '$window', '$timeout', 'Site', 'Messaging', 'SalesContactService', 'SalesAreaService', 'SalesContactTimelineService',
  ...
  var maphelper = {
    ...
    reverseGeocode: function(latlng, cb) {
      geocoder().geocode({
        "latLng": latlng
      }, function(results, status) {
        if (status !== gmaps.GeocoderStatus.OK) {
          cb("Geocoder fail status: " + status);
          return;
        }
        var data = {
          Latitude: latlng.lat(),
